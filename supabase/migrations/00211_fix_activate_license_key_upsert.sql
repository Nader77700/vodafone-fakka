...
BEGIN
  ...
  -- 4. تحديد الأيام الفعالة وتراكم الأيام
  v_effective_days := COALESCE(v_key.custom_duration_days, v_key.duration_days, 30);
  
  IF v_current_sub.id IS NOT NULL AND v_current_sub.status = 'active' AND v_current_sub.expires_at > v_now THEN
  ...
  -- 6. إضافة / تحديث الاشتراك باستخدام ON CONFLICT
  ...
  ON CONFLICT (user_id) DO UPDATE SET
    ...
    duration_days = COALESCE(subscriptions.duration_days, 0) + EXCLUDED.duration_days,
    days_remaining = COALESCE(subscriptions.days_remaining, 0) + EXCLUDED.days_remaining,
    ops_limit = COALESCE(EXCLUDED.ops_limit, subscriptions.ops_limit),
    ops_remaining = COALESCE(subscriptions.ops_remaining, 0) + EXCLUDED.ops_remaining,
  ...
  VALUES (p_user_id, v_key.id, v_key.code, COALESCE(v_key.code_type, 'paid'), v_effective_days,
          CASE WHEN v_expires_at > v_now THEN EXTRACT(DAY FROM (v_expires_at - v_now)) ELSE 0 END,
          EXTRACT(DAY FROM (v_final_expires - v_now)), v_now, v_final_expires, v_key.notes);

  RETURN jsonb_build_object('success', true, 'isTrial', v_is_trial, 'daysAfter', EXTRACT(DAY FROM (v_final_expires - v_now)));
