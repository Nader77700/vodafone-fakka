...
-- ============================================================
-- RPC: activate_license_key_v2
-- SECURITY DEFINER → يتجاوز RLS تمامًا لكل عمليات التفعيل
-- يُستدعى من الكلايينت بدلاً من التحديث المباشر
-- ============================================================
...
BEGIN
  ...
  IF v_key.code_type = 'paid' OR v_key.code_type IS NULL THEN
    ...
    IF v_is_multi_paid THEN
      ...
      END IF;
      -- زيادة used_count فقط (لا نغير status)
...
EXCEPTION WHEN OTHERS THEN
  -- تسجيل الخطأ الفعلي في system_logs
  INSERT INTO system_logs (user_id, level, action, message)
...
-- إتاحة RPC للمستخدمين المسجلين
GRANT EXECUTE ON FUNCTION activate_license_key_v2(UUID, TEXT) TO authenticated;
