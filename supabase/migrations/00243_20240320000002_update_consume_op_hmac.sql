...
BEGIN
  -- التحقق من التوقيع المشفر (HMAC) للطلب للتأكد من أنه قادم من التطبيق الرسمي غير المعدل
  v_is_valid_req := verify_request_signature();
  IF NOT v_is_valid_req THEN
    RETURN jsonb_build_object('allowed', false, 'reason', 'invalid_app_signature', 'ops_used', 0, 'ops_limit', 0, 'is_trial', false);
  END IF;
