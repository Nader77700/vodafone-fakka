ALTER TABLE device_gift_activations ADD COLUMN IF NOT EXISTS hardware_hash TEXT;
ALTER TABLE device_gift_activations ADD COLUMN IF NOT EXISTS native_id TEXT;

CREATE INDEX IF NOT EXISTS idx_dga_hardware_hash ON device_gift_activations(hardware_hash);
CREATE INDEX IF NOT EXISTS idx_dga_native_id ON device_gift_activations(native_id);
...
BEGIN
  ...
  ELSE
    INSERT INTO subscriptions (
      ...
      activated_at, expires_at, duration_days,
      ops_limit, ops_count, days_remaining
    ) VALUES (
      p_user_id, 'active', v_key.id, v_key.code,
      v_now, v_final_expires, v_days_after,
