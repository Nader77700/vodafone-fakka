...
BEGIN
  ...
  IF v_sub.code_type = 'trial' OR p_is_trial THEN
    ...
    RETURN jsonb_build_object('allowed', true, 'remaining', COALESCE(v_sub.ops_limit, 5) - v_new_used, 'is_trial', true, 'exhausted', v_new_used >= COALESCE(v_sub.ops_limit, 5));
  END IF;
  ...
  IF v_sub.ops_limit IS NOT NULL THEN
    ...
    END IF;
    UPDATE public.subscriptions SET ops_used = v_new_used WHERE id = v_sub.id;
    RETURN jsonb_build_object('allowed', true, 'remaining', v_sub.ops_limit - v_new_used, 'exhausted', v_new_used >= v_sub.ops_limit);
  ELSE
