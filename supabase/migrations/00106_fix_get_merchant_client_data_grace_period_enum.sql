-- إصلاح: إزالة 'grace_period' من CASE على enum subscription_status
-- الـ enum لا يحتوي على grace_period، بدلاً من ذلك يوجد عمود in_grace_period منفصل
...
BEGIN
  ...
  -- جلب الاشتراك النشط (إصلاح: استخدام status::text بدلاً من enum مباشرة لتجنب خطأ grace_period)
  ...
  IF FOUND AND v_sub_row.id IS NOT NULL THEN
    ...
    -- تحديد نوع الاشتراك من merchant_member_subscriptions إن وجد
    ...
    WHERE mms.subscription_id = v_sub_row.id
    LIMIT 1;
    ...
    RETURN jsonb_build_object(
      ...
      'subscription', jsonb_build_object(
        ...
        'ops_remaining',   CASE
                             WHEN v_sub_row.ops_limit IS NOT NULL
