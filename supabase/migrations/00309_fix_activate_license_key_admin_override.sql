-- ═══════════════════════════════════════════════════════════════════════
-- إصلاح activate_license_key — Admin Override يتخطى فحص status
-- المشكلة: الـ RPC ترفض الكود المستخدم (status='used') حتى لو adminOverride=TRUE
-- الإصلاح: عند adminOverride، نبحث عن الكود بغض النظر عن status
-- ═══════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ══════════════════════════════════════════════════════════════════════
  -- 1. البحث عن الكود
  --    Admin Override: يتخطى فحص status تماماً — يقبل active/used/disabled
  --    المستخدم العادي: يقبل active فقط
  -- ══════════════════════════════════════════════════════════════════════
  IF p_admin_override THEN
    -- الأدمن يبحث عن الكود بغض النظر عن حالته
    ...
    END IF;
    -- الأدمن لا يُرفض بسبب status='used' أو 'disabled'
    -- لكن يُرفض إذا الكود غير موجود أصلاً
  ELSE
    -- المستخدم العادي: يقبل active فقط
  ...
  --    Admin Override: يتخطى فحص انتهاء الصلاحية
  -- ══════════════════════════════════════════════════════════════════════
  ...
  -- ══════════════════════════════════════════════════════════════════════
  -- 3. حد المستخدمين
  --    Admin Override: يتخطى فحص حد المستخدمين
  -- ══════════════════════════════════════════════════════════════════════
  IF NOT p_admin_override THEN
  ...
  --    Admin Override: يتخطى كل فحوصات الجهاز والاستخدام السابق
  -- ══════════════════════════════════════════════════════════════════════
  ...
  IF FOUND THEN
    ...
    -- Admin Override يتخطى فحص الاشتراك التجريبي
  ...
  -- ══════════════════════════════════════════════════════════════════════
  -- 7. تحديث الكود — إعادة تفعيله إذا كان used (في حالة admin override)
  -- ══════════════════════════════════════════════════════════════════════
  v_max_allowed := COALESCE(v_key.allowed_users, v_key.max_users);
  UPDATE license_keys
  SET
