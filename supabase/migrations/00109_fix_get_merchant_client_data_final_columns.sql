-- إصلاح نهائي: تصحيح أسماء الأعمدة الفعلية في كل جدول
...
BEGIN
  ...
  -- FIX: status::text لتجنب خطأ grace_period في enum
  ...
  IF FOUND AND v_sub_row.id IS NOT NULL THEN
    ...
    SELECT mms.sub_type, mms.ops_limit,
           CASE WHEN mms.ops_limit IS NOT NULL
                ...
                ELSE NULL END
    INTO v_sub_type, v_ops_limit_val, v_ops_rem_val
    FROM merchant_member_subscriptions mms
    WHERE mms.user_id = p_user_id
    ORDER BY mms.created_at DESC
    LIMIT 1;

    -- fallback sub_type من license_key بالأعمدة الصحيحة
    ...
    -- استخدام الأعمدة الفعلية في subscriptions: ops_limit, ops_remaining
    v_ops_limit_val := COALESCE(v_ops_limit_val, v_sub_row.ops_limit);
    v_ops_rem_val   := COALESCE(v_ops_rem_val,   v_sub_row.ops_remaining);
    ...
    RETURN jsonb_build_object(
      ...
      'member', jsonb_build_object(
        ...
        'joined_at',  v_member.created_at   -- FIX: الاسم الصحيح هو created_at
      ),
