...
-- 1. إضافة DEFAULT 0 لعمود ops_used حتى لا يفشل INSERT بدونه
ALTER TABLE trial_usage ALTER COLUMN ops_used SET DEFAULT 0;

-- 2. تحديث RPC ليشمل ops_used = 0 صراحةً في INSERT
...
BEGIN
  ...
  IF v_key.code_type = 'paid' OR v_key.code_type IS NULL THEN
    ...
    ON CONFLICT (user_id) DO UPDATE SET license_key_id = EXCLUDED.license_key_id, status = 'active',
      expires_at = EXCLUDED.expires_at, in_grace_period = false, grace_started_at = NULL, grace_ends_at = NULL;
    ...
    VALUES (p_user_id,
      ...
      'الكود: ' || v_key.code || ' ✦ المدة: ' || v_effective_days || ' يوم', 'subscription_renewal', false, false);
  ...
  ELSIF v_key.code_type = 'trial' THEN
    ...
    INSERT INTO system_logs (user_id, level, action, message)
    VALUES (p_user_id, 'info', 'activate_trial', 'تم تفعيل كود تجريبي: ' || p_code || ' — ' || v_effective_days || ' يوم');
    IF p_device_fp IS NOT NULL AND p_device_fp <> '' THEN
  ...
  ELSIF v_key.code_type = 'gift' THEN
    ...
    VALUES (p_user_id, 'activation', 'استلام هدية ترحيبية',
            ...
            json_build_object('code', v_key.code, 'duration_days', v_effective_days));
    INSERT INTO system_logs (user_id, level, action, message)
    VALUES (p_user_id, 'info', 'activate_gift', 'تم تفعيل كود هدية: ' || p_code || ' — ' || v_effective_days || ' يوم');
    IF p_device_fp IS NOT NULL AND p_device_fp <> '' THEN
      INSERT INTO device_gift_activations (device_fp, user_id, username, license_key_id, code, code_type, activated_at)
