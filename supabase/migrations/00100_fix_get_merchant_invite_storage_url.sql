...
DECLARE
  ...
  v_locked     boolean;
  v_inv_code   text;
  v_app_base   text := 'https://vchmsnavyhripakyvzom.supabase.co/storage/v1/object/public/web-live/invite.html';
BEGIN
  ...
  -- استخدام app_base_url من app_config إن وُجد
  SELECT value INTO v_app_base FROM app_config WHERE key = 'app_base_url';
  IF v_app_base IS NULL OR v_app_base = '' THEN
    v_app_base := 'https://vchmsnavyhripakyvzom.supabase.co/storage/v1/object/public/web-live/invite.html';
  END IF;
