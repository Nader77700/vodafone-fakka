CREATE OR REPLACE FUNCTION execute_operation_transaction(
  p_payload JSONB,
  p_admin_override BOOLEAN DEFAULT FALSE -- This parameter is now IGNORED for security
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT role INTO v_caller_role FROM profiles WHERE id = v_user_id;
  ...
  IF NOT v_is_admin THEN
    ...
    END IF;
  ELSE
    v_consume_result := jsonb_build_object('allowed', true, 'code_type', 'admin');
  END IF;
