...
BEGIN
  ...
  v_new_used := COALESCE(v_sub.ops_used, 0) + 1;

  -- فحص نوع الاشتراك
  IF v_sub.code_type = 'trial' OR p_is_trial THEN
    IF v_sub.ops_used >= COALESCE(v_sub.ops_limit, 5) THEN
      RETURN jsonb_build_object('allowed', false, 'reason', 'trial_limit_reached', 'exhausted', true, 'is_trial', true, 'ops_used', v_sub.ops_used, 'ops_limit', COALESCE(v_sub.ops_limit, 5), 'code_type', v_sub.code_type);
    END IF;
    
    UPDATE public.subscriptions SET ops_used = v_new_used WHERE id = v_sub.id;
  ...
  IF v_sub.ops_limit IS NOT NULL THEN
    IF v_sub.ops_used >= v_sub.ops_limit THEN
      RETURN jsonb_build_object('allowed', false, 'reason', 'ops_limit_reached', 'exhausted', true, 'ops_used', v_sub.ops_used, 'ops_limit', v_sub.ops_limit, 'code_type', v_sub.code_type, 'is_trial', false);
    END IF;
    UPDATE public.subscriptions SET ops_used = v_new_used WHERE id = v_sub.id;
  ...
  ELSE
    ...
    IF v_sub.expires_at IS NOT NULL AND v_sub.expires_at < now() THEN
       RETURN jsonb_build_object('allowed', false, 'reason', 'subscription_expired', 'exhausted', true, 'ops_used', v_sub.ops_used, 'ops_limit', null, 'code_type', v_sub.code_type, 'is_trial', false);
    END IF;
    UPDATE public.subscriptions SET ops_used = v_new_used WHERE id = v_sub.id;
