...
BEGIN
  ...
  -- 6. حساب المدة (بما في ذلك أكواد التجربة التي تكون بالدقائق)
  IF v_key.type = 'trial' THEN
    -- trial_duration هنا تمثل الدقائق
  ...
  -- 7. قيود صارمة على جهاز Trial و Gift
  IF v_key.type IN ('trial', 'gift') AND NOT p_admin_override THEN
    IF EXISTS (
      ...
      RETURN json_build_object(
        'success', false, 
        'error', 'الجهاز مستخدم في حساب آخر (' || COALESCE(v_blocker_username, 'مجهول') || ')',
