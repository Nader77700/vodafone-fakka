...
-- ============================================================
-- ترقية activate_license_key_v2:
-- إضافة p_device_fp لمنع تعدد الحسابات على نفس الجهاز
-- ============================================================

CREATE OR REPLACE FUNCTION activate_license_key_v2(
  p_user_id   UUID,
  p_code      TEXT,
  p_device_fp TEXT DEFAULT NULL   -- ← جديد: بصمة الجهاز
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- ============================================================
  -- 3. فحص بصمة الجهاز لأكواد gift/trial فقط
  --    نُسجِّل device_fp في profiles أولاً لأي مستخدم
  -- ============================================================
  IF p_device_fp IS NOT NULL AND p_device_fp <> '' THEN
    -- تحديث device_fp للمستخدم الحالي (يُسجَّل دائماً)
    UPDATE profiles SET device_fp = p_device_fp WHERE id = p_user_id;

    -- للأكواد المجانية فقط: تحقق إن كان الجهاز استُخدم من قبل
  ...
  -- ============================================================
  -- 4. جلب الاشتراك الحالي + الأيام المتبقية
  -- ============================================================
  ...
  IF v_key.code_type = 'paid' OR v_key.code_type IS NULL THEN
    ...
    ELSE
      ...
      END IF;
      UPDATE license_keys
      SET status = 'used', used_by = p_user_id, used_at = v_now, used_count = 1
      WHERE id = v_key.id;
    END IF;
  ...
  ELSIF v_key.code_type = 'trial' THEN
    ...
    INSERT INTO trial_usage (user_id, license_key_id, activated_at, expires_at)
    VALUES (p_user_id, v_key.id, v_now, v_final_expires)
    ...
    -- ── تسجيل بصمة الجهاز بعد نجاح التفعيل التجريبي ──
    IF p_device_fp IS NOT NULL AND p_device_fp <> '' THEN
      INSERT INTO device_gift_activations
        (device_fp, user_id, username, license_key_id, code, code_type, activated_at)
      SELECT p_device_fp, p_user_id, COALESCE(pr.username, ''),
             v_key.id, v_key.code, 'trial', v_now
  ...
  ELSIF v_key.code_type = 'gift' THEN
    ...
    -- ── تسجيل بصمة الجهاز بعد نجاح تفعيل الهدية ──
    IF p_device_fp IS NOT NULL AND p_device_fp <> '' THEN
      INSERT INTO device_gift_activations
        (device_fp, user_id, username, license_key_id, code, code_type, activated_at)
      SELECT p_device_fp, p_user_id, COALESCE(pr.username, ''),
             v_key.id, v_key.code, 'gift', v_now
      FROM   profiles pr WHERE pr.id = p_user_id
      ON CONFLICT DO NOTHING;
    END IF;
...
GRANT EXECUTE ON FUNCTION activate_license_key_v2(UUID, TEXT, TEXT) TO authenticated;
