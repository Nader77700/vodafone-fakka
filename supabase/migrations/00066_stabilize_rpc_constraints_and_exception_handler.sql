...
-- ============================================================
-- تثبيت النظام نهائياً: constraints + EXCEPTION handler آمن
-- ============================================================

-- 1. مزامنة السجلات القديمة: key_id → license_key_id
UPDATE trial_usage
SET license_key_id = key_id
WHERE license_key_id IS NULL AND key_id IS NOT NULL;

-- 2. إضافة UNIQUE(license_key_id, user_id) لـ trial_usage
--    حتى ON CONFLICT يعمل حتى لو key_id فارغ مستقبلاً
CREATE UNIQUE INDEX IF NOT EXISTS idx_trial_usage_license_user
  ON trial_usage(license_key_id, user_id)
  WHERE license_key_id IS NOT NULL;

-- 3. إضافة UNIQUE لـ device_gift_activations حتى ON CONFLICT يعمل
CREATE UNIQUE INDEX IF NOT EXISTS idx_dga_device_user_code
  ON device_gift_activations(device_fp, user_id, license_key_id)
  WHERE user_id IS NOT NULL AND license_key_id IS NOT NULL;

-- 4. تحديث EXCEPTION block ليكون آمناً تماماً حتى لو FK فشل
...
BEGIN
  ...
  IF p_device_fp IS NOT NULL AND p_device_fp <> '' THEN
    ...
    IF v_key.code_type IN ('gift', 'trial') THEN
      SELECT dga.username, dga.user_id INTO v_blocker_username, v_blocker_user_id
      FROM device_gift_activations dga
      WHERE dga.device_fp = p_device_fp AND dga.user_id <> p_user_id LIMIT 1;
      IF FOUND THEN
  ...
  ELSIF v_key.code_type = 'trial' THEN
    ...
    END IF;
    -- يتحقق من كلا العمودين للتوافق مع السجلات القديمة والجديدة
    IF EXISTS (SELECT 1 FROM trial_usage WHERE user_id = p_user_id AND (license_key_id = v_key.id OR key_id = v_key.id)) THEN
    ...
    v_days_after := v_days_before + v_effective_days;
    UPDATE license_keys SET used_count = used_count + 1 WHERE id = v_key.id;
    -- يملأ key_id و license_key_id و ops_used معاً
    INSERT INTO trial_usage (user_id, key_id, license_key_id, ops_used, activated_at, expires_at)
  ...
  ELSIF v_key.code_type = 'gift' THEN
    ...
    VALUES (p_user_id, v_key.id, v_now) ON CONFLICT DO NOTHING;
    INSERT INTO subscriptions (user_id, license_key_id, status, activated_at, expires_at, in_grace_period, grace_started_at, grace_ends_at)
    VALUES (p_user_id, v_key.id, 'active', v_now, v_final_expires, false, NULL, NULL)
    ON CONFLICT (user_id) DO UPDATE SET
      license_key_id = EXCLUDED.license_key_id, status = 'active',
...
-- ================================================================
-- EXCEPTION آمن تماماً: لا يفشل حتى لو system_logs أخطأ
-- ================================================================
EXCEPTION WHEN OTHERS THEN
  ...
  EXCEPTION WHEN OTHERS THEN
    NULL; -- تجاهل خطأ الـ logging نفسه (مثلاً FK violation)
  END;
