-- ══════════════════════════════════════════════════════════════════
-- إصلاح atomic_insert_operation_and_consume:
-- expires_at = NULL يعني unlimited → يجب قبوله دون شرط المقارنة
-- ══════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- FIX: expires_at = NULL يعني unlimited → اقبله بدون شرط التاريخ
  SELECT id INTO v_sub_id
  FROM subscriptions
  WHERE user_id = p_user_id
    AND status  = 'active'
    AND (expires_at IS NULL OR expires_at > COALESCE(p_performed_at, now()))
