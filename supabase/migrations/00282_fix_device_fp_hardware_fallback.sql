...
DECLARE
  ...
  v_ops_limit        integer;
BEGIN
  -- 0. تطبيع معرّفات الجهاز: نتجاهل القيم الفارغة أو الـ fallback المشتركة
  IF p_device_fp = '' THEN p_device_fp := NULL; END IF;
  IF p_hardware_hash = '' OR p_hardware_hash = 'hw-fallback' THEN p_hardware_hash := NULL; END IF;
  IF p_native_id = '' THEN p_native_id := NULL; END IF;
  ...
  ON CONFLICT (user_id) DO UPDATE SET
    ...
    days_remaining = EXCLUDED.duration_days,
