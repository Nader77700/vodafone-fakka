...
BEGIN
  ...
  UPDATE license_keys
  SET
    status = CASE WHEN v_is_trial THEN 'active' ELSE 'used' END, -- التجريبي يظل active حتى يصل للحد الأقصى
    used_by = p_user_id,
    activated_at = v_now,
    updated_at = v_now,
  ...
  INSERT INTO subscriptions (
    ...
    p_user_id, v_key.id, 'active', v_key.code_type, v_effective_days, v_effective_days,
