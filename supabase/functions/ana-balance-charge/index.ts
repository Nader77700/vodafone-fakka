...
serve(async (req: Request) => {
  ...
  try {
    ...
    const abortAndRefund = async (payload: any) => {
      if (!operationRefunded) {
        await supabaseAdmin.rpc('atomic_refund_operation', { p_user_id: caller.id });
        operationRefunded = true;
      }
      return json(payload);
    };
    ...
    if (appBuild < 356 && legacyBlockedProducts.includes(product_id)) {
      console.log("[balance-charge] legacy product blocked for old versions:", product_id);
      return await abortAndRefund({ success: false, error: "تم إيقاف هذا المنتج للإصدارات القديمة. يرجى تحديث التطبيق إلى أحدث إصدار." });
    }
    ...
    if (!receiver.startsWith("01") || receiver.length !== 11)
      return await abortAndRefund({ success: false, error: "رقم المستفيد غير صحيح — 11 رقم يبدأ بـ 01" });
    ...
    await supabaseAdmin.from("operations").insert({
      user_id: caller.id, phone_number: receiver, card_type: productConfig?.display_name || product_id,
      category: "فكة", amount: productConfig?.price || 0, status: "failed",
