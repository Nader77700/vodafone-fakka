...
export function useSubscriptionEngine(opts?: { intervalMs?: number }): SubscriptionEngineState {
  ...
  const isUnlimited = (isActive && !subscription?.expires_at && opsInfo?.opsLimit === null);

  // ── اللون الموحّد ────────────────────────────────────────────────────────
  const timeLeftObj = fmtTimeLeft(subscription?.expires_at);
  const planColor = !isActive              ? '#ef4444'
    : timeLeftObj.status === 'critical'  ? '#ef4444'
    : timeLeftObj.status === 'expiring'  ? '#F7C948'
    : '#22c55e';

  // ── حالة موحّدة ─────────────────────────────────────────────────────────
  const status: SubStatus = !subscription ? 'none'
    : !isActive               ? 'expired'
  ...
  const expiresAt    = !expiresAtRaw   ? (isActive ? 'غير محدود ♾️' : '—')
    : fmtDateAr(expiresAtRaw);

  const progressPct = !subscription?.expires_at ? 0
    : isActive
  ...
  return {
    ...
    planName,
    planColor,
    timeLeft: isActive ? timeLeftObj : { label: 'منتهي', color: '#ef4444', status: 'expired' },
    activatedAt,
    expiresAt,
    progressPct,
    opsUsed,
