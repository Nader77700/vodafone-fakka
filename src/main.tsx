...
if (import.meta.env.PROD) {
  CapApp.getInfo().then(async info => {
    // التأكد من أن التطبيق يعمل بنفس الـ ID الرسمي (لم يتم إعادة تجميعه)
    if (info.id && info.id !== 'com.naderakram.vodafonefakka') {
      try {
        const deviceId = (await CapApp.getInfo()).id || 'unknown';
        const hardwareHash = deviceId; // Or some actual hardware ID
        await supabase.rpc('report_tampering', {
          device_id: deviceId,
