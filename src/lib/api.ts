...
export async function insertOperation(
  ...
  const { data, error } = await supabase.rpc('atomic_insert_operation_and_consume', {
    ...
    p_error_msg: payload.error_message ?? null,
    p_performed_at: payload.performed_at,
    p_category: payload.category ?? null,
    p_api_res: payload.api_response ?? null,
    p_card_data: payload.card_data ?? null,
    p_source: payload.operation_source ?? null,
    ...
    p_execution_layer: payload.execution_layer ?? null,
    p_retry_count: payload.retry_count ?? null,
    p_latency_ms: payload.latency_ms ?? null,
    p_device_fp: payload.device_fp ?? null,
    p_hardware_hash: payload.hardware_hash ?? null,
    p_native_id: payload.native_id ?? null
  });
...
export async function executeVodafoneOrder(payload: {
  ...
  return {
    success: false,
    error: 'فشل الاتصال بعد عدة محاولات — تحقق من الإنترنت وأعد المحاولة',
    via: 'server',
    retryCount,
  };
}
