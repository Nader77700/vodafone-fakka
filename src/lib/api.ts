...
export async function executeVodafoneOrder(payload: {
...
}): Promise<{ success: boolean; error?: string; via?: 'native' | 'bridge' | 'server'; debugSteps?: ChargeDebugStep[]; retryCount?: number }> {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (data.success) return { success: true, error: data.error, via: 'server', retryCount };
    if (isPermanentError(data.error)) return { success: false, error: data.error, via: 'server', retryCount };
    if (isTransientError(data.error) && attempt < MAX_RETRIES) continue;
    return { success: false, error: data.error, via: 'server', retryCount };
  }
...
/** فحص الحسابات المفقودة — profiles موجودة في DB بدون Auth record */
...
/** بناء رابط الدعوة الكامل من التوكن */
