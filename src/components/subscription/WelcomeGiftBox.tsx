    ...
    return (
      <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border transition-all ${
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground">{new Date(entry.claimed_at).toLocaleDateString('en-GB')}{days > 0 ? ` · ${days} يوم` : ''}</p>
        </div>
        {code && (
          <button
    ...
    <div className="rounded-xl border border-border/60 bg-card overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-border/40">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs bg-primary/10 text-primary px-1.5 py-0.5 rounded-full font-bold">{gifts.length}</span>
        </div>
      ...
      {claimed.length > 0 && (
        <div className="border-t border-border/40">
          <button
            ...
            className="w-full flex items-center justify-between px-3 py-2 text-xs text-muted-foreground hover:text-foreground hover:bg-muted/30 transition-all"
          >
            <span className="font-semibold flex items-center gap-1.5">
              <Check className="w-3 h-3 text-success" />
              الهدايا المستفادة ({claimed.length})
            </span>
            <span className="text-xs">{showClaimed ? '▲ إخفاء' : '▼ عرض'}</span>
          </button>
          {showClaimed && (
