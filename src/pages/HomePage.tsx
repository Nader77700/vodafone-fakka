...
function ExecuteModal({
  ...
  const handleExecute = async () => {
    ...
    // تم إزالة checkAndConsumeOperation من الفرونت إند ليكون الحسم Server-side حصراً
...
export default function HomePage() {
  ...
  const loadData = () => {
    ...
    Promise.all([
      ...
      if (ops_info?.isExhaustedByUsage && sub?.status === 'active') {
    ...
    <div className="pb-6 space-y-0 page-enter" dir="rtl">
      ...
      <div className="px-4 pt-3">
        <SubscriptionAlertBanner
          ...
          isAdmin={false}
          opsRem={opsInfo?.opsLimit != null ? Math.max(0, opsInfo.opsLimit - (opsInfo.opsUsed ?? 0)) : null}
          opsLimit={opsInfo?.opsLimit ?? null}
          exhaustedByUsage={!!(opsInfo?.isExhaustedByUsage)}
            ...
            <div className="grid grid-cols-2 gap-2.5">
              ...
              {[
                ...
                },
                {
                  ...
                  label: opsInfo?.opsLimit !== null && opsInfo?.opsLimit != null
                      ? 'المتبقي من العمليات'
                      : 'الحد الشهري',
                  value: opsInfo?.opsLimit == null
                      ? 'غير محدود ♾️'
                      : opsInfo.opsLimit != null
                        ? `${Math.max(0, opsInfo.opsLimit - (opsInfo.opsUsed ?? 0))} / ${opsInfo.opsLimit}`
                        : '—',
                  color: opsInfo?.opsLimit == null ? '#00C896'
                ...
                {
                  label: isUnlimited ? 'نوع الخطة' : 'الوقت المتبقي',
                  value: isUnlimited ? '∞ غير محدود'
                    : subActive ? liveTime.label
                    : subscription?.in_grace_period ? 'فترة سماح'
                    : 'منتهٍ',
                  color: isUnlimited ? '#00C896'
                    : !subActive ? '#ef4444'
                    : liveTime.color,
                  icon: Clock,
                  glow: !isUnlimited && subActive && (liveTime.status === 'critical' || liveTime.status === 'expiring'),
                },
