...
function OperationDetailsDialog({
  ...
  const rows = [
    { label: 'رقم العملية',       value: receipt.opNumber != null ? `#${receipt.opNumber}` : '—',  copyable: true },
    { label: 'رقم المستلم',       value: receipt.receiverPhone,   copyable: true },
    { label: 'الفئة',             value: receipt.category,        copyable: false },
    { label: 'الكارت',            value: receipt.productName,     copyable: false },
    { label: 'السعر',             value: receipt.productPrice,    copyable: false },
    { label: 'الحالة',            value: 'ناجحة ✅',               copyable: false },
    { label: 'تاريخ التنفيذ',    value: receipt.date,            copyable: false },
    { label: 'وقت التنفيذ',      value: receipt.time,            copyable: false },
    { label: 'طريقة التنفيذ',    value: receipt.via,             copyable: true  },
    { label: 'رقم المحفظة',      value: receipt.walletPhone,     copyable: true  },
    { label: 'مدة التنفيذ',      value: `${receipt.latencyMs} ms`, copyable: false },
    { label: 'Correlation ID',   value: receipt.correlationId,   copyable: true  },
  ];
        ...
        <div className="p-4 space-y-2">
          {rows.map(r => (
            <div key={r.label}
              ...
              <div className="flex items-center gap-2 min-w-0">
                <span className="text-xs font-bold text-white font-mono truncate max-w-[160px]">{r.value}</span>
                {r.copyable && r.value !== '—' && (
                  <button
...
// ── مكوّن: شاشة الإيصال بعد نجاح الشحن ──
function ReceiptView({
  receipt, onChargeAnother, onClose,
}: {
  receipt: {
    opNumber: number | null;
    receiverPhone: string;
    ...
    <div className="flex flex-col" dir="rtl">
      ...
      <div className="flex flex-col items-center gap-3 px-5 py-6"
        ...
        <div className="w-16 h-16 rounded-full flex items-center justify-center"
          style={{ background: 'rgba(34,197,94,0.15)', border: '2px solid rgba(74,222,128,0.4)' }}>
          <CheckCircle2 className="w-8 h-8 text-green-400" />
        </div>
        <div className="text-center space-y-0.5">
          <p className="text-base font-black text-green-400">✓ تمت العملية بنجاح</p>
          {receipt.opNumber != null && (
            <p className="text-xs font-mono" style={{ color: 'rgba(255,255,255,0.4)' }}>
              رقم العملية: <span className="text-white font-bold">#{receipt.opNumber}</span>
            </p>
          )}
        </div>
      </div>
      ...
      <div className="px-5 pb-2 space-y-2">
        {[
          { label: 'رقم الهاتف المشحون', value: receipt.receiverPhone,  accent: true  },
          { label: 'الكارت',             value: receipt.productName,     accent: false },
          { label: 'القيمة',             value: receipt.productPrice,    accent: true  },
          { label: 'الفئة',             value: receipt.category,        accent: false },
          { label: 'وقت التنفيذ',       value: receipt.time,            accent: false },
          { label: 'تاريخ التنفيذ',     value: receipt.date,            accent: false },
          { label: 'رقم المحفظة',       value: receipt.walletPhone,     accent: false },
          { label: 'الحالة',            value: 'ناجحة',                  accent: false },
          { label: 'طريقة التنفيذ',     value: `Native ${receipt.via}`, accent: false },
        ].map(r => (
          <div key={r.label}
            className="flex items-center justify-between gap-2 px-3 py-2 rounded-xl"
            style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
            <span className="text-[11px]" style={{ color: 'rgba(255,255,255,0.4)' }}>{r.label}</span>
            <span className={`text-xs font-bold font-mono ${r.accent ? 'text-green-400' : 'text-white'}`}>{r.value}</span>
          </div>
        ))}
      </div>
      ...
      {/* نافذة التفاصيل */}
      <OperationDetailsDialog open={detailsOpen} onClose={() => setDetailsOpen(false)} receipt={receipt} />
    </div>
  );
}
...
function ExecuteModal({
  ...
  const handleExecute = async () => {
    ...
    // إشعار واحد فقط — dedup_key يمنع التكرار لنفس العملية
...
export default function HomePage() {
  ...
  const prefillPhone = (location.state as { prefillPhone?: string } | null)?.prefillPhone ?? '';
  ...
  return (
    <div className="pb-6 space-y-0 page-enter" dir="rtl">
      <ExpiryModal open={isExpired && !subscription?.in_grace_period} reason="expired" />

      {/* ── شريط فترة السماح — يظهر للمستخدمين الذين انتهى اشتراكهم ولم تنته ساعة السماح ── */}
      {!isAdmin && subscription?.in_grace_period && subscription.grace_ends_at && (
      ...
      {/* تنبيه انتهاء الاشتراك — يقرأ من Engine فقط */}
      <div className="px-4 pt-3">
