...
export default function InvitePage() {
  ...
  // دعم /invite/:token (path param) + ?merchant=TOKEN (query param)
  const resolvedToken = token || searchParams.get('merchant') || '';
  ...
  useEffect(() => {
    if (!resolvedToken) { setState('invalid'); setErrorMsg('رابط غير صالح'); setErrorSub('لم يتم تقديم توكن.'); return; }

    (async () => {
      const res = await validateInviteToken(resolvedToken);
