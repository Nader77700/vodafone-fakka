...
export default function TransferHistoryPage() {
  ...
  return (
    <div className="min-h-screen bg-background pb-24 font-cairo">
      {/* ── Top Nav ── */}
      <div className="sticky top-0 z-50 bg-background/92 backdrop-blur-xl border-b border-border shadow-sm">
        <div className="flex items-center justify-between px-4 h-16">
          ...
          <div className="flex-1 text-center">
            <h1 className="text-[17px] font-bold tracking-wide text-foreground">سجل التحويل</h1>
        ...
        <div className="space-y-3">
          {history.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center space-y-3 opacity-60">
              ...
              <p className="text-sm font-medium text-muted-foreground">لا توجد عمليات تحويل مسجلة بعد</p>
            </div>
          ) : (
            history.map((op) => (
              <div key={op.id} className="bg-card border border-border rounded-xl p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{new Date(op.created_at).toLocaleDateString('ar-EG')}</p>
                  </div>
                </div>
