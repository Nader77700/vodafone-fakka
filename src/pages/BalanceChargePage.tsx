...
function BalanceExecuteDialog({
  ...
  const handleExecute = async () => {
    ...
    await logActivity(user.id, 'recharge',
      success ? `شحن ناجح (رصيد أنا فودافون) — ${product.display_name}` : `شحن فاشل (رصيد أنا فودافون) — ${product.display_name}`,
      `الرقم: ${receiverPhone} | المبلغ: ${product.price} جنيه | المصدر: الشحن من الرصيد${opNumber != null ? ` | #${opNumber}` : ''}`,
      { product_id: product.product_id, phone: receiverPhone, amount: product.price, status: success ? 'success' : 'failed', operation_source: 'ana_vodafone_balance', operation_number: opNumber }
    );
    await insertSystemLog({
      user_id: user.id, level: success ? 'info' : 'warning',
      action: success ? 'balance_recharge_success' : 'balance_recharge_failed',
      message: success
        ? `شحن ناجح (رصيد أنا فودافون) — ${product.display_name} — ${product.price} جنيه — ${receiverPhone}${opNumber != null ? ` — #${opNumber}` : ''}`
        : `شحن فاشل (رصيد أنا فودافون) — ${product.display_name} — ${receiverPhone} — ${(errorMsg ?? '').split('\n')[0]}`,
      metadata: { product_id: product.product_id, phone: receiverPhone, amount: product.price, operation_source: 'ana_vodafone_balance', operation_number: opNumber },
    });
    await sendNotification({
      user_id: user.id,
      title: success ? `✅ تم شحن ${product.display_name} (رصيد أنا فودافون)` : `❌ فشل الشحن (رصيد أنا فودافون) — ${product.display_name}`,
      body: success
        ? `المستخدم: ${username}\nالرقم: ${receiverPhone}${opNumber != null ? `\nرقم العملية: #${opNumber}` : ''}\nالتاريخ: ${dateLabel}\nالوقت: ${timeLabel}\nالمصدر: الشحن من الرصيد (أنا فودافون)\nالحالة: ناجحة`
