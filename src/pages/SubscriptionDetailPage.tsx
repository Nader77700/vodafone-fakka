...
export default function SubscriptionDetailPage() {
  ...
  // Admin يُعدّ دائماً نشطاً بصرف النظر عن أي subscription row في DB
  const subActive  = isAdmin || !!(subscription?.status === 'active'
    && (!subscription?.expires_at || new Date(subscription.expires_at).getTime() > Date.now()));
  // غير محدود = نشط فعلاً + لا تاريخ انتهاء + لا حد عمليات
  const isUnlimited = isAdmin || (subActive && !subscription?.expires_at && opsInfo?.opsLimit === null);
      ...
      <div className="sticky top-0 z-20 px-4 pt-4 pb-3"
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-bold"
            ...
            {isAdmin ? 'نشط دائماً' : subActive ? 'نشط' : 'منتهي'}
          </div>
        </div>
      </div>
      ...
      <div className="px-4 space-y-4">
        ...
        <Section title="معلومات الاشتراك" icon={CreditCard} color={statusColor}>
          <div className="space-y-0">
            ...
            <InfoRow label="الحالة" value={isAdmin ? 'نشط دائماً' : subActive ? 'نشط' : 'منتهي'} color={statusColor} />
            {!isAdmin && (
            ...
            <InfoRow
              label="الوقت المتبقي"
              value={isAdmin ? 'غير محدود ♾️' : isUnlimited ? 'غير محدود ♾️' : subActive ? timeLeft.label : 'منتهي'}
              color={isAdmin ? '#00E5FF' : isUnlimited ? '#00C896' : subActive ? timeLeft.color : '#ef4444'}
            />
            {!isAdmin && (
          ...
          {!isAdmin && !isUnlimited && subscription?.expires_at && (
