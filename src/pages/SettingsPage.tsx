        ...
        {eng.loading ? (
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between p-3 rounded-xl"
              ...
              <div className="flex items-center gap-2">
                <Crown className="w-4 h-4" style={{ color: '#00E5FF' }} />
                <span className="text-sm font-black" style={{ color: '#00E5FF' }}>مسؤول النظام</span>
              </div>
              <span className="text-xs font-bold px-2 py-1 rounded-full"
                style={{ background: '#00E5FF18', color: '#00E5FF' }}>
                {profile?.role === 'super_admin' ? 'مدير عام' : 'مسؤول'}
              </span>
            </div>
            {[
              { icon: Shield, label: 'نوع الخطة',    val: 'مسؤول النظام' },
              { icon: Shield, label: 'الحالة',        val: 'نشط دائماً' },
              { icon: Infinity, label: 'الحد الشهري', val: 'غير محدود ♾️' },
              { icon: Infinity, label: 'الوقت المتبقي', val: 'غير محدود ♾️' },
              ...
              <div key={label} className="flex items-center justify-between py-1.5 border-b border-border last:border-0">
                ...
                <span className="text-sm font-semibold" style={{ color: '#00E5FF' }}>{val}</span>
              </div>
            ))}
          </div>
        ) : eng.status === 'none' ? (
