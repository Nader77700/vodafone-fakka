/**
 * WalletLinesLoginPage — Login UI — PHASE 1 (بدون API)
 * رقم هاتف + كلمة مرور. Validation كامل. Mock فقط.
 * Navigation Flow: Login → National ID Verification
 */
...
export default function WalletLinesLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // Service Layer — لا API مباشر في UI
      ...
      setSuccess(true);
      // Navigation Flow: Login → National ID
    ...
    <div className="min-h-screen pb-28 flex flex-col" dir="rtl"
      ...
      <form onSubmit={handleSubmit} className="flex-1 px-4 pt-6 flex flex-col gap-5" noValidate>
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            ...
            <Input
              ...
              value={phone}
              onChange={e => { setPhone(e.target.value); if (phoneErr) setPhoneErr(null); setErrorMsg(null); }}
              onBlur={() => setPhoneErr(validatePhone(phone))}
              disabled={isDisabled}
              maxLength={11}
              className="pr-9 text-left placeholder:text-right font-mono"
              dir="ltr"
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            ...
            <Input
              ...
              value={password}
              onChange={e => { setPassword(e.target.value); if (passErr) setPassErr(null); setErrorMsg(null); }}
        ...
        <Button
          type="submit"
          disabled={isDisabled}
          className="w-full h-12 text-sm font-black rounded-xl mt-2 relative overflow-hidden"
        ...
        >
          {loading ? (
            ...
            <span className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4" /> تم الدخول</span>
          ) : 'تسجيل الدخول'}
        </Button>
        ...
        <p className="text-center text-xs text-white/40">
          ليس لديك حساب؟{' '}
          <button type="button" onClick={() => navigate('/wallet-lines/register')}
            disabled={isDisabled}
        ...
        {/* Navigation Flow hint — مرجعي فقط */}
