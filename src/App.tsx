...
// لمنع استغلال كود الهدية/التجريبي على حسابات متعددة من نفس الجهاز
function DeviceFingerprintRegistrar() {
  ...
  useEffect(() => {
    ...
    registerDeviceFingerprint(user.id, fp);
  }, [user]);
  return null;
}
...
function AppInner() {
  ...
  const isColdStart = (() => {
    const ts = Number(localStorage.getItem(ACTIVITY_KEY) ?? 0);
    if (!ts) return true;                            // أول تشغيل على الإطلاق
    return Date.now() - ts > RESUME_TTL_MS;          // غياب طويل = cold start
  })();
