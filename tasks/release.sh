...
if command -v supabase &> /dev/null; then
  # ربط المشروع (إذا لم يكن مرتبطاً)
  supabase link --project-ref vchmsnavyhripakyvzom 2>/dev/null || true
  # نشر serve-app بدون JWT — يسمح للـ APK بالوصول بدون apikey
  supabase functions deploy serve-app \
    --project-ref vchmsnavyhripakyvzom \
    --no-verify-jwt 2>&1 | tail -3
  echo "✅ serve-app منشور (no-verify-jwt) — Live URL Mode مفعَّل"
else
  echo "⚠️  Supabase CLI غير موجود — تخطي نشر serve-app"
...
SK        = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZjaG1zbmF2eWhyaXBha3l2em9tIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4MjI4Nzg1NSwiZXhwIjoyMDk3ODYzODU1fQ.qGv6iURGQONn7wlG55S8HMCxTfodI2GQfcV4PkpARIo"
